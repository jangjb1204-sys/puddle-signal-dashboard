...
def main() -> None:
    ...
    st.title("Puddle Signal Scanner")
    st.caption("상위 주식/ETF에서 Puddle 및 RSI & Puddle 신호를 확인합니다.")
    ...
    if result.empty:
        st.info(
            "저장된 결과가 없거나 해당 날짜에 신호가 없습니다. "
            "왼쪽의 Update on GitHub에서 workflow를 실행하면 CSV가 repo에 저장됩니다."
        )
    else:
